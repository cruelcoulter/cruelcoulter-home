...
function my_str_split($string) { $slen=strlen($string); for($i=0; $i
...
function makeSlugs($string, $maxlen=0) { $newStringTab=array(); $string=strtolower(noDiacritics($string)); if(function_exists('str_split')) { $stringTab=str_split($string); } else { $stringTab=my_str_split($string); }
...
echo sluggable("Ron Coulter 1965-2018"); 
?>
